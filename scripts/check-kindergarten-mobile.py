"""Check real candidate/baseline Vite builds. No forms or live analytics are sent."""
...
OVERFLOW='''root => [...root.querySelectorAll('h1,h2,h3,p,li,button,summary,a,video,img')].filter(el => {
 ...
 return r.left < -1 || r.right > innerWidth+1 || (el.clientWidth>0 && el.scrollWidth>el.clientWidth+2);
...
def prepare(context,url,width):
 ...
 page.locator('h1').first.wait_for();return page

def dimensions(page):
 return page.evaluate('''() => [...document.querySelectorAll('main > section, main h1, main h2, #albums')].map(el=>{
 const r=el.getBoundingClientRect(),s=getComputedStyle(el);
 return {tag:el.tagName,text:el.tagName==='SECTION'?'':el.textContent.trim(),x:r.x,y:r.y+scrollY,w:r.width,h:r.height,font:s.fontSize,color:s.color};})''')

def reveal(page):
 page.evaluate('''async()=>{for(let y=0;y<document.body.scrollHeight;y+=600){scrollTo(0,y);await new Promise(r=>setTimeout(r,20));}scrollTo(0,0)}''')
 page.wait_for_timeout(250)

try:
 with sync_playwright() as p:
  browser=p.chromium.launch(); context=browser.new_context(reduced_motion='reduce')
  context.add_init_script('window.__qaGoals=[];window.ym=(...args)=>window.__qaGoals.push(args)')
  ...
  for width in (320,360,390,430,767):
   page=prepare(context,AFTER+'/kindergarten?utm_source=qa&utm_campaign=mobile_v1',width)
   assert page.locator('#albums').count()==1,'Duplicate albums anchor'
   assert page.locator('.km-choice').count()==5,'Five formats required'
   ...
   assert not catalog.evaluate(OVERFLOW),f'Overflow at {width}: {catalog.evaluate(OVERFLOW)}'
   for album_id,price in [('folder','2 700 ₽'),('trio','2 950 ₽'),('six-pages','3 300 ₽'),('ten-pages','3 900 ₽'),('fourteen-pages','6 500 ₽')]:
    ...
    assert not catalog.evaluate(OVERFLOW),f'Overflow {album_id} at {width}'
   ...
   if width in (320,390):
    ...
    assert not catalog.evaluate(OVERFLOW),f'200% text overflow at {width}: {catalog.evaluate(OVERFLOW)}'
    page.screenshot(path=str(OUT/f'after-text-200-{width}.png'))
   old=prepare(context,BEFORE+'/kindergarten',width)

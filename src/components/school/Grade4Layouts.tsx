...
const grade4Layouts: Grade4Layout[] = [
  ...
  { slug: "cheat-sheet", title: "Шпаргалка", imageCount: 11 },
  { slug: "foxes", title: "Лисы", imageCount: 11 },
  { slug: "giraffes", title: "Жирафики", imageCount: 9 },
];

const Grade4LayoutImage = ({
  layout,
  imageNumber,
  className,
  eager = false,
  ...
  const basePath = `/layouts-grade4/${layout.slug}/${imageNumber}`;
  ...
  return (
    <picture>
      ...
      <img
        src={`${basePath}.webp`}
        alt={`${layout.title} — пример оформления выпускного альбома для 4 класса, страница ${imageNumber}`}
        className={className}
    ...
    <section id="layouts" className="bg-background py-20">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-foreground md:text-5xl">Макеты альбомов для 4 класса</h2>
        ...
        <div ref={gridRef} className="mx-auto grid max-w-6xl gap-7 sm:grid-cols-2 lg:grid-cols-3">
          {grade4Layouts.map((layout, index) => (
